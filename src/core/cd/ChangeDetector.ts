	...
	static onChange = (
		...
		const handler = {
			get(
				...
				if (
					isPrimitive(value) ||
					ChangeDetector.isBuiltinWithoutMutableMethods(value) ||
				...
				return buildProxy(
					value,
					ChangeDetector.concatPath(pathCache!.get(target), property)
				);
			},
			...
			apply(target: any, thisArg: any, argumentsList: any) {
				...
				if (!inApply) {
					...
					if (
						changed ||
						(compare && !equals(applyPrevious, thisArg.valueOf()))
					) {
						handleChange(
							applyPath,
							"",
