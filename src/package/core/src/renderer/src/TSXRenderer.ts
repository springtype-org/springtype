...
import {Component, ConsoleLogger} from "../../index";
import {
    Attribute,
    checkNameNs,
...
export class TSXRenderer implements IRenderer {
    ...
    render = (reactCreateElementOrTagName: IReactCreateElement|string, level = 0, namespaces: Namespace[] = []): Element => {
        //this.logger.trace('render', reactCreateElement, level, namespaces);
        const name = typeof reactCreateElementOrTagName === 'string' ? reactCreateElementOrTagName : reactCreateElementOrTagName.name;
        const attributes = (reactCreateElementOrTagName as IReactCreateElement).attributes || {};
        const children = (reactCreateElementOrTagName as IReactCreateElement).children || [];
